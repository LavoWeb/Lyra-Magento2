...
class Payment
{
    ...
    const IDENTIFIER = 'lyra_identifier';

    // Key to save choosen multi option.
    const MULTI_OPTION = 'lyra_multi_option';

    // Key to save choosen Choozeo option.
    const CHOOZEO_OPTION = 'lyra_choozeo_option';

    // Key to save choosen Oney option.
    const ONEY_OPTION = 'lyra_oney_option';

    // Key to save choosen Full CB option.
    const FULLCB_OPTION = 'lyra_fullcb_option';

    // Key to save risk control results.
    const RISK_CONTROL = 'lyra_risk_control';

    // Key to save risk assessment results.
    const RISK_ASSESSMENT = 'lyra_risk_assessment';

    // Key to save risk assessment results.
    const ALL_RESULTS = 'lyra_all_results';

    const TRANS_UUID = 'lyra_trans_uuid';

    const BRAND_USER_CHOICE = 'lyra_brand_user_choice';
     ...
     * @param \Lyranetwork\Lyra\Model\Api\LyraResponse $response
     */
    public function registerOrder(
        ...
        if ($method instanceof \Lyranetwork\Lyra\Model\Method\Sepa)  {
    ...
    public function nextOrderState(
        ...
        if ($response->isToValidatePayment()) {
            ...
            if ($this->isSepa($response)) {
                ...
                $newStatus = 'lyra_pending_transfer';
            } else {
    ...
    public function updatePaymentInfo(\Magento\Sales\Model\Order $order, \Lyranetwork\Lyra\Model\Api\LyraResponse $response)
    {
        ...
        $order->getPayment()
            ...
            ->setAdditionalInformation(\Lyranetwork\Lyra\Helper\Payment::ALL_RESULTS, serialize($response->getAllResults()))
            ->setAdditionalInformation(\Lyranetwork\Lyra\Helper\Payment::TRANS_UUID, $response->get('trans_uuid'));
        ...
        if ($response->get('brand_management')) {
            ...
            $order->getPayment()->setAdditionalInformation(\Lyranetwork\Lyra\Helper\Payment::BRAND_USER_CHOICE, $userChoice);
        }
        ...
        if ($response->get('card_brand') == 'MULTI') { // Multi brand.
            ...
            $option = @unserialize($order->getPayment()->getAdditionalInformation(\Lyranetwork\Lyra\Helper\Payment::MULTI_OPTION));

            if ($response->get('sequence_number') == 1 && (stripos($order->getPayment()->getMethod(), 'lyra_multi') === 0)
    ...
    public function saveIdentifier(
        ...
        if ($response->get('identifier') && (
            ...
            $customerData->setCustomAttribute('lyra_identifier', $response->get('identifier'));

            // Mask card number and save it to customer entity.
            $customerData->setCustomAttribute('lyra_masked_pan', $this->maskPan($response));

            try {
                $customer->updateData($customerData);

                $customerResource = $this->customerResourceFactory->create();
                $customerResource->saveAttribute($customer, 'lyra_identifier');
                $customerResource->saveAttribute($customer, 'lyra_masked_pan');
    ...
    public function saveSepaIdentifier(
        ...
        if ($response->get('identifier') && (
            ...
            $customerData->setCustomAttribute('lyra_sepa_identifier', $response->get('identifier'));

            // Mask IBAN and save it to customer entity.
            $customerData->setCustomAttribute('lyra_sepa_iban_bic', $this->maskPan($response));

            try {
                $customer->updateData($customerData);

                $customerResource = $this->customerResourceFactory->create();
                $customerResource->saveAttribute($customer, 'lyra_sepa_identifier');
                $customerResource->saveAttribute($customer, 'lyra_sepa_iban_bic');
     ...
     *
     * @param string $lyraType
     * @return string
     */
    public function convertTransactionType($lyraType)
    {
        $type = false;
