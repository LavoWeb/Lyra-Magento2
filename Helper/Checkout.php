...
class Checkout
{
    ...
    public function checkCustormers($scope, $scopeId)
    {
        ...
        foreach ($collection as $customer) {
            if (! preg_match(self::CUST_ID_REGEX, $customer->getId())) {
                ...
                $msg = '';
                $msg .= __(
                    'Customer ID &laquo; %1 &raquo; does not match Lyra Collect specifications.',
                    $customer->getId()
                )->render() . ' ';
    ...
    private function checkOrderId($orderId)
    {
        if (! preg_match(self::ORDER_ID_REGEX, $orderId)) {
            ...
            $msg = '';
            $msg .= __(
                'The next order ID  &laquo; %1 &raquo; does not match Lyra Collect specifications.',
                $orderId
            )->render() . ' ';
    ...
    public function checkProducts($scope, $scopeId)
    {
        ...
        foreach ($collection as $product) {
            if (! preg_match(self::PRODUCT_REF_REGEX, $product->getId())) {
                ...
                $msg = '';
                $msg .= __(
                    'Product reference &laquo; %1 &raquo; does not match Lyra Collect specifications.',
                    $product->getId()
                )->render() . ' ';
    ...
    public function toLyraCategory($categoryIds)
    {
    ...
    public function setCartData($order, &$lyraRequest)
    {
        $notAllowed = '#[^A-Z0-9ÁÀÂÄÉÈÊËÍÌÎÏÓÒÔÖÚÙÛÜÇ ]#ui';

        // Used currency.
        $currency = LyraApi::findCurrencyByNumCode($lyraRequest->get('currency'));
        ...
        foreach ($order->getAllItems() as $item) {
            ...
            if (! $item->getParentItem()) {
                ...
                $lyraRequest->addProduct(
                    preg_replace($notAllowed, ' ', $label),
                    $priceInCents,
                    $qty,
                    $item->getProductId(),
                    $this->toLyraCategory($product->getCategoryIds())
                );
        ...
        $lyraRequest->set('insurance_amount', 0); // By default, shipping insurance amount is not available in Magento.
        $lyraRequest->set('shipping_amount', $currency->convertAmountToInteger($order->getShippingAmount()));

        // Recalculate tax_amount to avoid rounding problems.
        $taxAmount = $lyraRequest->get('amount') - $subtotal - $lyraRequest->get('shipping_amount') -
             $lyraRequest->get('insurance_amount');
        if ($taxAmount <= 0) { // When order is discounted.
            $taxAmount = $currency->convertAmountToInteger($order->getTaxAmount());
        }

        $lyraRequest->set('tax_amount', $taxAmount);

        // VAT amount for colombian payment means.
        $lyraRequest->set('totalamount_vat', $taxAmount);
    }

    public function setOneyData($order, &$lyraRequest)
    {
        // By default, clients are private.
        $lyraRequest->set('cust_status', 'PRIVATE');
        $lyraRequest->set('ship_to_status', 'PRIVATE');
        ...
        if ($order->getIsVirtual() || ! $order->getShippingMethod()) {
            ...
            $lyraRequest->set('ship_to_type', 'ETICKET');
            $lyraRequest->set('ship_to_speed', 'EXPRESS');
        } else {
            ...
            switch ($shippingMethod['type']) {
                ...
                case 'RECLAIM_IN_STATION':
                    ...
                    $lyraRequest->set('ship_to_street', $address);
                    $lyraRequest->set('ship_to_zip', $order->getShippingAddress()->getPostcode());
                    $lyraRequest->set('ship_to_city', $order->getShippingAddress()->getCity());
                    $lyraRequest->set('ship_to_country', 'FR');
                    $lyraRequest->set('ship_to_street2', null); // Not sent to FacilyPay Oney.
                    $lyraRequest->set('ship_to_state', null);
                    $lyraRequest->set('ship_to_phone_num', null);
                    ...
                    $lyraRequest->set('ship_to_delivery_company_name', $method);
                    break;
                default:
                    $address = '';
                    $address .= $order->getShippingAddress()->getStreet(1);
                    ...
                    $lyraRequest->set('ship_to_delivery_company_name', $shippingMethod['oney_label']);
                    break;
            }

            $lyraRequest->set('ship_to_type', $shippingMethod['type']);
            $lyraRequest->set('ship_to_speed', $shippingMethod['speed']);
        }
    }
