...
define(
    ...
    function ($, Component, url, storage, quote, setPaymentInformationAction, additionalValidators, errorProcessor, fullScreenLoader) {
        ...
        return Component.extend({
            defaults: {
                template: 'Lyranetwork_Lyra/payment/lyra-standard',
                lyraCcType:  window.checkoutConfig.payment.lyra_standard.availableCcTypes ?
                    window.checkoutConfig.payment.lyra_standard.availableCcTypes[0]['value'] : null,
                lyraUseIdentifier: 1,
                payload: null,
            ...
            initObservable: function () {
                ...
                this.observe('lyraUseIdentifier');
            ...
            getData: function () {
                ...
                if (this.isOneClick()) {
                    data['additional_data']['lyra_standard_use_identifier'] = this.lyraUseIdentifier();
                }

                if (this.getEntryMode() == 2) {
                    data['additional_data']['lyra_standard_cc_type'] = this.lyraCcType();
                }
            ...
            getRestFormToken: function () {
                return window.checkoutConfig.payment.lyra_standard.restFormToken || null;
            },

            getLanguage: function () {
                return window.checkoutConfig.payment.lyra_standard.language || null;
            },

            updatePaymentBlock: function (blockName) {
                $('.payment-method._active .payment-method-content .lyra-identifier li.lyra-block').hide();
                $('li.lyra-standard-' + blockName + '-block').show();
            },
            ...
            initRestEvents: function (elts) { // To be called after KR script loading.
                ...
                KR.onFormReady(function (e) {
                    $('#lyra_rest_form .kr-iframe-wrapper').css('display', '');
                });

                KR.onFocus(function(e) {
                    $('#lyra_rest_form .kr-form-error').html('');
                });
                ...
                KR.onError(function (e) {
                    ...
                    $('#lyra_rest_form .kr-form-error').html('<span style="color: red;"><span>' + msg + '</span></span>');
                });
            },
            ...
            placeOrderClick: function () {
                ...
                if ((me.getEntryMode() == 4) && me.getRestFormToken()) {
                    ...
                    if (me.payload && (me.payload === newPayload)) {
                        $('#lyra_rest_form .kr-payment-button').click();
                    } else {
            ...
            refreshToken: function () {
                ...
                storage.post(
                    url.build('lyra/payment_rest/token?form_key=' + $.mage.cookies.get('form_key'))
            ...
            initIframeEvents: function (elts) { // To be called after iframe loading.
                ...
                quote.paymentMethod.subscribe(function(method) {
                    if ((method.method === 'lyra_standard') || !me.iframeDisplayed) {
                        return;
                    }

                    var iframe = $('.payment-method .payment-method-content iframe.lyra-iframe');
                    if (iframe && iframe.length) {
                        var cancelUrl = me.getIframeLoaderUrl() + '?mode=cancel' + '&' + Date.now();
                        iframe.attr('src', cancelUrl);

                        $('.payment-method .payment-method-content .lyra-iframe').hide();
                        $('.payment-method .payment-method-content .lyra-form').show();
            ...
            afterPlaceOrder: function () {
                ...
                if (me.getEntryMode() == 3) {
                    ...
                    $('.payment-method._active .payment-method-content .lyra-form').hide();
                    $('.payment-method._active .payment-method-content .lyra-iframe').show();

                    var iframe = $('.payment-method._active .payment-method-content iframe.lyra-iframe');
                    if (iframe && iframe.length) {
