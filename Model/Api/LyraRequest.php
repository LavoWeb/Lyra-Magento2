...
if (! class_exists('LyraRequest', false)) {
    ...
    class LyraRequest
    {
         ...
         * @access private
         */
        private $algo = LyraApi::ALGO_SHA1;
        ...
        {
            // Initialize encoding.
            $this->encoding = in_array(strtoupper($encoding), LyraApi::$SUPPORTED_ENCODINGS) ?
            ...
            $timestamp = time();
            $this->set('vads_trans_id', LyraApi::generateTransId($timestamp));
        ...
        {
            $this->requestParameters[$name] = new LyraField($name, $label, $regex, $required, $length);
        ...
        {
            if (in_array($algo, LyraApi::$SUPPORTED_ALGOS)) {
                $this->algo = $algo;
                return true;
            }
        ...
        /**
         * Generate signature from a list of LyraField.
         *
         * @param array[string][LyraField] $fields already filtered fields list
         * @param bool $hashed
         * @return string
         */
        private function generateSignature($fields, $hashed = true)
        {
            ...
            return LyraApi::sign($params, $this->getCertificate(), $this->algo, $hashed);
        }
         ...
         *
         * @return array[string][LyraField] a list of LyraField
         */
