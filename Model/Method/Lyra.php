...
abstract class Lyra extends \Magento\Payment\Model\Method\AbstractMethod
{

    const CART_MAX_NB_PRODUCTS = 85;

    protected $_infoBlockType = \Lyranetwork\Lyra\Block\Payment\Info::class;
    ...
    /**
     *
     * @var \Lyranetwork\Lyra\Model\Api\LyraRequest
     */
    protected $lyraRequest;

    /**
     *
     * @var \Lyranetwork\Lyra\Model\Api\LyraResponse
     */
    protected $lyraResponse;
    ...
    /**
     *
     * @var \Lyranetwork\Lyra\Helper\Checkout
     */
    ...
    public function __construct(
        ...
        $this->lyraRequest = $lyraRequestFactory->create();
    ...
    {
        // Set order_id.
        $this->lyraRequest->set('order_id', $order->getIncrementId());

        // Amount in current order currency.
        $amount = $order->getGrandTotal();

        // Set currency.
        $currency = LyraApi::findCurrencyByAlphaCode($order->getOrderCurrencyCode());
        if ($currency == null) {
            // If currency is not supported, use base currency.
            $currency = LyraApi::findCurrencyByAlphaCode($order->getBaseCurrencyCode());

            // ... and order total in base currency
            $amount = $order->getBaseGrandTotal();
        }
        $this->lyraRequest->set('currency', $currency->getNum());

        // Set the amount to pay.
        $this->lyraRequest->set('amount', $currency->convertAmountToInteger($amount));
        ...
        $this->lyraRequest->set('contrib', $cmsParam . '/' . $cmsVersion . '/' . PHP_VERSION);
        ...
        foreach ($configFields as $field) {
            $this->lyraRequest->set($field, $this->dataHelper->getCommonConfigData($field));
        }

        // Check if capture_delay and validation_mode are overriden in submodules.
        if (is_numeric($this->getConfigData('capture_delay'))) {
            $this->lyraRequest->set('capture_delay', $this->getConfigData('capture_delay'));
        }

        if ($this->getConfigData('validation_mode') !== '-1') {
            $this->lyraRequest->set('validation_mode', $this->getConfigData('validation_mode'));
        }
        ...
        $this->lyraRequest->set('url_return', $returnUrl);

        // Set the language code.
        $this->lyraRequest->set('language', $this->getPaymentLanguage());
        ...
        $this->lyraRequest->set('available_languages', $availableLanguages);
        ...
        $this->lyraRequest->set('threeds_mpi', $threedsMpi);

        $this->lyraRequest->set('cust_email', $order->getCustomerEmail());
        $this->lyraRequest->set('cust_id', $order->getCustomerId());
        $this->lyraRequest->set('cust_title', $order->getBillingAddress()->getPrefix() ?
            $order->getBillingAddress()->getPrefix() : null);
        $this->lyraRequest->set('cust_first_name', $order->getBillingAddress()->getFirstname());
        $this->lyraRequest->set('cust_last_name', $order->getBillingAddress()->getLastname());
        $this->lyraRequest->set('cust_address', implode(' ', $order->getBillingAddress()->getStreet()));
        $this->lyraRequest->set('cust_zip', $order->getBillingAddress()->getPostcode());
        $this->lyraRequest->set('cust_city', $order->getBillingAddress()->getCity());
        $this->lyraRequest->set('cust_state', $order->getBillingAddress()->getRegion());
        $this->lyraRequest->set('cust_country', $order->getBillingAddress()->getCountryId());
        $this->lyraRequest->set('cust_phone', $order->getBillingAddress()->getTelephone());
        $this->lyraRequest->set('cust_cell_phone', $order->getBillingAddress()->getTelephone());

        $address = $order->getShippingAddress();
        if (is_object($address)) { // Shipping is supported.
            $this->lyraRequest->set('ship_to_first_name', $address->getFirstname());
            $this->lyraRequest->set('ship_to_last_name', $address->getLastname());
            $this->lyraRequest->set('ship_to_city', $address->getCity());
            $this->lyraRequest->set('ship_to_street', $address->getStreetLine(1));
            $this->lyraRequest->set('ship_to_street2', $address->getStreetLine(2));
            $this->lyraRequest->set('ship_to_state', $address->getRegion());
            $this->lyraRequest->set('ship_to_country', $address->getCountryId());
            $this->lyraRequest->set('ship_to_phone_num', $address->getTelephone());
            $this->lyraRequest->set('ship_to_zip', $address->getPostcode());
        }
        ...
        if ($sendCartDetails || $this->needsCartData /* Cart data are mandatory for the payment method. */) {
            $this->checkoutHelper->setCartData($order, $this->lyraRequest);
        }

        if ($this->sendOneyFields()) {
            // Set other data specific to FacilyPay Oney payment.
            $this->checkoutHelper->setOneyData($order, $this->lyraRequest);
        }

        $paramsToLog = $this->lyraRequest->getRequestFieldsArray(true);
        $this->dataHelper->log('Payment parameters: ' . json_encode($paramsToLog), \Psr\Log\LogLevel::DEBUG);

        return $this->lyraRequest->getRequestFieldsArray(false, false);
    }
    ...
    public function resetData()
    {
        ...
        $keys = [
            \Lyranetwork\Lyra\Helper\Payment::MULTI_OPTION,
            \Lyranetwork\Lyra\Helper\Payment::CHOOZEO_OPTION,
            \Lyranetwork\Lyra\Helper\Payment::FULLCB_OPTION,
            \Lyranetwork\Lyra\Helper\Payment::ONEY_OPTION,
    ...
    public function acceptPayment(\Magento\Payment\Model\InfoInterface $payment)
    {
        ...
        try {
            ...
            $uuid = $payment->getAdditionalInformation(\Lyranetwork\Lyra\Helper\Payment::TRANS_UUID);
            if (! $uuid) { // Retro compatibility.
                $legacyTransactionKeyRequest = new \Lyranetwork\Lyra\Model\Api\Ws\LegacyTransactionKeyRequest();
                ...
                $getPaymentUuid = new \Lyranetwork\Lyra\Model\Api\Ws\GetPaymentUuid();
            ...
            $queryRequest = new \Lyranetwork\Lyra\Model\Api\Ws\QueryRequest();
            $queryRequest->setUuid($uuid);

            $getPaymentDetails = new \Lyranetwork\Lyra\Model\Api\Ws\GetPaymentDetails($queryRequest);
            ...
            $wrapper = new \Lyranetwork\Lyra\Model\Api\Ws\ResultWrapper(
                $getPaymentDetailsResponse->getGetPaymentDetailsResult()->getCommonResponse(),
            ...
            exit;
        } catch(\Lyranetwork\Lyra\Model\WsException $e) {
            $this->dataHelper->log(
                "[$requestId] {$e->getMessage()}",
                \Psr\Log\LogLevel::WARNING
            );

            $this->messageManager->addWarning(__('Please fix this error to use Lyra Collect web services.'));
    ...
    public function denyPayment(\Magento\Payment\Model\InfoInterface $payment)
    {
        ...
        try {
            ...
            $cancelPayment = new \Lyranetwork\Lyra\Model\Api\Ws\CancelPayment();
            $cancelPayment->setCommonRequest(new \Lyranetwork\Lyra\Model\Api\Ws\CommonRequest());
            ...
            if ($e->getCode() === -1) {
                ...
                $notice = __('You are not authorized to do this action online. Please, do not forget to update payment in Lyra Expert Back Office.');
    ...
    public function validatePayment(\Magento\Payment\Model\InfoInterface $payment)
    {
        ...
        try {
            ...
            $multi = (stripos($payment->getMethod(), 'lyra_multi') === 0) && is_array($option) && !empty($option);
            $count = $multi ? (int) $option['count'] : 1;

            // Retrieve saved transaction UUID.
            $savedUuid = $payment->getAdditionalInformation(\Lyranetwork\Lyra\Helper\Payment::TRANS_UUID);
            ...
            for ($i = 1; $i <= $count; $i++) {
                ...
                $validatePayment = new \Lyranetwork\Lyra\Model\Api\Ws\ValidatePayment();
                $validatePayment->setCommonRequest(new \Lyranetwork\Lyra\Model\Api\Ws\CommonRequest());
                ...
                $wrapper = new \Lyranetwork\Lyra\Model\Api\Ws\ResultWrapper($validatePaymentResponse->getValidatePaymentResult()->getCommonResponse());
    ...
    public function canUseForCurrency($baseCurrencyCode)
    {
        ...
        if ($quote && $quote->getId()) {
            ...
            $currency = LyraApi::findCurrencyByAlphaCode($currencyCode);
            if ($currency) {
                return true;
            }
        }
        ...
        $currency = LyraApi::findCurrencyByAlphaCode($baseCurrencyCode);
        if ($currency) {
            return true;
        }
     ...
     * @param \Magento\Payment\Model\InfoInterface $payment
     * @param float $amount
     * @return Lyranetwork\Lyra\Model\Method\Lyra
     */
    public function refund(\Magento\Payment\Model\InfoInterface $payment, $amount)
    {
        ...
        try {
            ...
            $getPaymentDetails = new \Lyranetwork\Lyra\Model\Api\Ws\GetPaymentDetails();
            $getPaymentDetails->setQueryRequest($queryRequest);
            ...
            $commonRequest = new \Lyranetwork\Lyra\Model\Api\Ws\CommonRequest();
            ...
            if ($transStatus === 'CAPTURED') { // Transaction captured, we can do refund.
                ...
                $paymentRequest = new \Lyranetwork\Lyra\Model\Api\Ws\PaymentRequest();
                $paymentRequest->setTransactionId(LyraApi::generateTransId($timestamp));
                ...
                $refundPayment = new \Lyranetwork\Lyra\Model\Api\Ws\RefundPayment();
                ...
                if ($amountInCents >= $transAmount) { // Transaction cancel.
                    ...
                    $updatePayment = new \Lyranetwork\Lyra\Model\Api\Ws\UpdatePayment();
            ...
            $this->messageManager->addError($e->getMessage());
            $this->messageManager->addWarning('Please fix error to refund payments through Lyra Collect. If you want to refund order in Magento, use the &laquo; Refund Offline &raquo; button.');
            throw new \Exception($e->getMessage());
        } catch (\SoapFault $f) {
            $this->dataHelper->log(
                "[$requestId] SoapFault with code {$f->faultcode}: {$f->faultstring}.",
    ...
    {
        $currency = LyraApi::findCurrencyByNumCode($paymentResponse->getCurrency());
        ...
        if ($paymentResponse->getEffectiveCurrency() &&
            ...
            $effectiveCurrency = LyraApi::findCurrencyByNumCode($paymentResponse->getEffectiveCurrency());
    ...
    protected function checkAndGetWsApi($storeId)
    {
        ...
        $options = parse_ini_file($this->dirReader->getModuleDir('etc', 'Lyranetwork_Lyra') . '/ws.ini') ?: [];
        ...
        $wsApi = new \Lyranetwork\Lyra\Model\Api\Ws\WsApi($url, $options);
